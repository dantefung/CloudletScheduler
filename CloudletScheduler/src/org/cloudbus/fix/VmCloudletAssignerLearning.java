...
public class VmCloudletAssignerLearning extends VmCloudletAssigner { //强化学习策略
	...
	public VmCloudletAssignerLearning( double gamma, double alpha, double epsilon, GenExcel genExcel) {
		this.gamma = gamma;
		this.alpha = alpha;
...
//				System.exit(0);
			...
			for (i = 0; i < n; i++) { //分配任务到适合的虚拟机
				int index = createAction(numFreeVm, tmpSizeList);//选择动作，即被选中的虚拟机的id号，对应Q值表中的列号
...
//				int id = vmWaitingQueueSizeList.get(index).get("id");
				...
				if (vQueueSize.increment(id)) { //决断是否能正确分配到被选中的虚拟机中，虚拟机的子队列队长队长加一
					...
					/*
					 * index:被选中的虚拟机的id号--即Q值表的列号
					 * m:虚拟机列表的长度
					 * vmList:虚拟机列表
					 * vmWaitingQueueSizeList:虚拟机上的云任务缓存队列
					 * 
					 **/
...
//		/////////////////
	...
	private void updateQList(int action_idx, int numVm, List<Vm> vmList, List<Map<String, Integer>> vmWaitingQueueSizeList) { //更新Q值表
		double sample = ((QCloudletSchedulerSpaceShared) vmList                   //被选中的虚拟机中的任务平均等待时间
				.get(action_idx).getCloudletScheduler()).getAverageWaitingTime();
		if (sample == 0.0) {          //若这个值为0，初始化为足够大的数
			sample = 1000000.0;
		}
		double reward = 1.0 / sample; //由被选中的虚拟机中的任务平均等待时间的倒数生成的reward值
