...
import org.cloudbus.cloudsim.QDatacenter;
...
public class QDatacenterBroker extends DatacenterBroker {
	...
	public void startEntity() {	// 本代理开始工作
		...
		setVmList(createVM(getId(), numVm, vmMips, 0)); // 初始化虚拟机
	...
	private static List<Vm> createVM(int userId, int vms,int[] vmMips, int idShift) {// 创建虚拟机
		...
		for (int i = 0; i < vms; i++) {
			vm[i] = new Vm(idShift + i, userId, vmMips[i], pesNumber, ram, bw, size,
